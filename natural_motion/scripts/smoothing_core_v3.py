...
def generate_smooth_trajectory_with_limits(input_plan, jerk_profile_type, total_duration, jerk_min=-1.0, jerk_max=None):
	# type: (moveit_msgs.msg.RobotTrajectory, function, float, float, float) -> moveit_msgs.msg.RobotTrajectory
	...
	**Warning**: this function should not be used to produce a usable trajectory to command a robot. It is instead intended for testing and/or generating models for one joint at a time, since all joints will have the same position goal.
	...
	times = np.linspace(0.0, total_duration, len(jerk_profile) + 1, retstep=True)
